...
resetprop_and_write ro.build.id AP3A.240617.008
resetprop_and_write ro.build.version.incremental 1747495579146
...
for prefix in "" bootimage system product odm system_ext; do
    ...
    resetprop_and_write "${product_prefix}.brand" oplus
    resetprop_and_write "${product_prefix}.name" ossi
    resetprop_and_write "${product_prefix}.device" ossi
    resetprop_and_write "${product_prefix}.model" ossi
    resetprop_and_write "${product_prefix}.manufacturer" oplus

    resetprop_and_write "${product_prefix}.brand_for_attestation" oplus
    resetprop_and_write "${product_prefix}.name_for_attestation" ossi
    resetprop_and_write "${product_prefix}.device_for_attestation" ossi
    resetprop_and_write "${product_prefix}.model_for_attestation" ossi
    resetprop_and_write "${product_prefix}.manufacturer_for_attestation" oplus
done
