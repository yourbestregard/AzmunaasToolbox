...
# Menjalankan logika yang sama untuk setiap direktori di dalam TARGET_DIRS
for KEYBOX_DIR in $TARGET_DIRS; do
    log_message "======================================="
    log_message "Processing target directory: $KEYBOX_DIR"
    
    KEYBOX_FILE="$KEYBOX_DIR/keybox.xml"
    KEYBOX_BACKUP_FILE="$KEYBOX_DIR/keybox_backup.xml"

    # Pastikan direktori tujuan sudah ada, jika belum maka buat direktori tersebut
    if [ ! -d "$KEYBOX_DIR" ]; then
        log_message "Directory does not exist. Creating $KEYBOX_DIR..."
        mkdir -p "$KEYBOX_DIR"
    fi
    ...
    if [ -f "$KEYBOX_FILE" ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            log_message "ERROR: Failed to backup keybox.xml in $KEYBOX_DIR."
        fi
    fi

    # Try downloading the new keybox using the new download function
    log_message "Processing keybox from URL for $KEYBOX_DIR..."
    ...
    if [ "$DOWNLOAD_EXIT_CODE" -eq 0 ] && [ -f "$KEYBOX_FILE" ] && [ -s "$KEYBOX_FILE" ]; then
        ...
        if [ -f "$KEYBOX_BACKUP_FILE" ]; then
            ...
            if [ $? -eq 0 ]; then
                ...
                log_message "CRITICAL: Failed to restore backup keybox in $KEYBOX_DIR. Module might not work."
            fi
        else
            log_message "CRITICAL: Download failed and no backup file was found in $KEYBOX_DIR."
        fi
    fi
done

log_message "======================================="
log_message "All keybox installations completed."